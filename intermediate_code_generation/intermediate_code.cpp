...
void _E ( E * that ) { } 
 ...
 void set_x ( A * that , int x ) { that -> x = x ; } 
  void foo ( ) { long kk ; } 
 int v ( A * that , int j ) { return 333 ; } 
 void v_function ( A * that , int a , int v ) { } 
 void ese_virtuaL ( D * that , int v ) { int l ; } 
 int main ( ) { A obj4 ; A * pobj = new A ( ) ; pobj->v_ptr = (A_vtable*)&Atable ; set_x(&obj4,3) ; ; pobj->v_ptr->v(pobj,2) ; } 