...
namespace constants
{
    const char _private = 'e' ;
    const char _protected = 'd' ;
    const char _public = 'c' ;
} 
...
bool generator::is_type(const std::string & str) const
{
    bool fundomental_type = [&]()->bool{ 
        return  (str == "int" || str == "floath" || str == "char" || str == "double"  || str == "string") ? true : false ;
    }() ;
    auto my_type = std::find_if(vec_c_class.cbegin(),vec_c_class.cend(),[&](const c_class & ob){
...
bool generator::obj_or_pointer (const std::string& str, const strings_pairs& class_obj_names, const strings_pairs& pointers)
{
...
void generator::generate_user_coll(std::vector<std::string> & vec,const strings_pairs & class_obj_names , const strings_pairs& pointers)
{
    for(int i = 0 ; i < vec.size() ; ++i)
    {
        bool pointer {false} ;
        auto pos = std::find_if(class_obj_names.cbegin(),class_obj_names.cend(),[&](const std::pair<std::string,std::string> & str) {
            if(obj_or_pointer(vec[i], class_obj_names, pointers) && (vec[i + 1] == "." ||  vec[i + 1] == "->"))
            {
                if (vec[i + 1] == "->") {
                    pointer = true ; 
                }
            ...
            std::for_each(vec_c_class.begin(),vec_c_class.end(),[&](c_class & obj) {
                std::find_if(obj.members.begin(),obj.members.end(),[&](std::tuple<char,std::string,std::string,std::string> & member) {
                    if(std::get<2>(member) == vec[i + 2]) {
                        if(std::get<0>(member) == constants::_private) { std::cerr << "hangstaci hly es JS chi" << std::endl ; throw ; }
                ...
                std::for_each(obj.function.begin() , obj.function.end() ,[&] (std::pair<char,std::vector<std::string>>& f) {
                    ...
                    {
                        if(f.first == constants::_private) { std::cerr << "hangstaci hly es JS chi" << std::endl ; throw ; } 
                        std::string obname = vec[i] ;
                        if(obj.class_name != pos->first) {
                            obname += ".subObj" ;
                        }
                        std::string foo_name = vec[i + 2] ;
                        ...
                        if(!pointer) {
                            vec.insert(pos , foo_name + "(&" + obname + str_tmp) ;
                        } else {
                            obj.v_table.virtual_function_coll(vec,pos,foo_name,obname,str_tmp) ;
                            // vec.insert(pos , foo_name + "(" + obname + str_tmp) ;
                        }
                    }
                }) ;
...
void generator::generate_c_base_pointer(std::vector<std::string> & vec, const strings_pairs& class_obj_names, const strings_pairs& pointers) 
{
    ...
    for(int i = 0 ; i < vec.size() ; ++i)
    {
        ...
        if(pointer_position != pointers.end())
        {
            ...
            std::for_each(vec.begin() + i + 1 , line_pos,[&](std::string& str) {
                if(obj_or_pointer(str,class_obj_names,pointers)) {
...
void generator::generate_user(std::vector<std::string> & vec)
{
    ...
    generate_user_coll(vec, class_obj_names, pointers) ;
...
{   
    if(obj.v_table.have_virtual_function)
    {
        ...
        obj.v_table.generate_virtual_functions(ofile) ;
        ofile << "}" + obj.class_name + "table;" << std::endl ;
        obj.members.push_back(std::make_tuple(constants::_public, obj.class_name + "_vtable*", "v_ptr", ";")) ;
    }
    ...
    for(int i = 0; i < vec.size(); ++i) 
    {
        if(vec[i] == "(") {
            ...
            if(*(vec.begin()) == "virtual" && pos != vec.end()) {    // virtual funtions
                ...
                // std::cout << *(pos_parenthes );
                obj.v_table.set_fuction(std::vector<std::string>(vec.begin() + i - 2,pos_parenthes + 1) , obj.class_name, *this) ;
...
void virtual_table::set_fuction(const std::vector<std::string>& fun,const std::string& class_name , const generator & typ) 
{
    ...
    for(int i = 3 ; i < fun.size() ; ++i)
    {
        if(typ.is_type(fun[i]))
        {
            table_fun.push_back(",") ;
            table_fun.push_back(fun[i]) ;
        }
